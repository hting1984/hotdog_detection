 ...
 */
 ...
 // Get information about the memory area to use for the model's input.
  ...
  TF_LITE_MICRO_EXPECT_EQ(4, input->dims->size);   // 1,96,96,1
  TF_LITE_REPORT_ERROR(error_reporter,"confidence:%d",input->dims->size);
 ...
 //const float* hotdog_data = g_hotdog_data;
...
/*  TfLiteStatus invoke_status = interpreter.Invoke();
 ...
 // TF_LITE_MICRO_EXPECT_EQ(kTfLiteOk, invoke_status);
  ...
  TF_LITE_REPORT_ERROR(error_reporter,
                       "confidence:%f  non-confidence:%f",
                       output->data.f[0],output->data.f[1]);
